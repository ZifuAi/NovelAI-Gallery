...
novelai-gallery/
  ...
  tests/        Browser test suites (./tests/run.sh runs everything)
  docs/         HANDOFF.md — project state and what to do next
  CLAUDE.md     Working notes: conventions, traps, and why things are so
```
...
```
./tests/run.sh              # Go tests + every browser suite
./tests/run.sh folderui     # just one
```

The browser suites need Playwright (`npm install playwright && npx
playwright install chromium`). Each starts its own copy of the app with a
throwaway data directory and drives the real interface, so nothing touches
your own library.
